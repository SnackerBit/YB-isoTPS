...
def _compute_overlap_except_W_prime_1(T1, T2, W, T1_prime, T2_prime, U):
    ...
    Returns
    -------
    contr: backend.array_type of shape (l', (u', d'), r')
        result of the contraction.
    """
    contr = backend.tensordot(T2, backend.conj(T2_prime), ([1], [1])) # j [(ru rd)] l; j* [(ru rd)*] l* -> j l j* l*
    contr = backend.tensordot(contr, U, ([0, 2], [3, 1])) # [j] l [j*] l*: i [j] i* [j*] -> l l* i i*
    contr = backend.tensordot(contr, W, ([0], [2])) # [l] l* i i*; l (u d) [r] -> l* i i* l (u d)
    temp = backend.tensordot(T1, backend.conj(T1_prime), ([2], [2])) # i r [(ld lu)]; i* r* [(ld lu)*] -> i r i* r*
    contr = backend.tensordot(contr, temp, ([1, 2, 3], [2, 0, 1])) # l* [i] [i*] [l] (u d); [i] [r] [i*] r* -> l* (u d) r* = l (u d) r
...
def _compute_overlap_1(T1, T2, W, T1_prime, T2_prime, W_prime, U):
    ...
    T1, T2, W: backend.array_types
        two site wavefunction psi. For the shapes of the individual tensors, see tebd_step_1().
    T1_prime, T2_prime, W_prime: backend.array_types
        two site wavefunction psi_prime. For the shapes of the individual tensors, see tebd_step_1().
    U: backend.array_type of shape (i, j, i, j)
        two-site (time evolution) operator.
    ...
    return backend.tensordot(contr, backend.conj(W_prime), ([0, 1, 2], [0, 1, 2]))
...
def _compute_error_1(T1, T2, W, T1_prime, T2_prime, W_prime, U):
    ...
    temp = _compute_overlap_1(T1, T2, W, T1, T2, W, backend.tensordot(U, backend.conj(U), ([2, 3], [2, 3]))) # i j [i*] [j*]; i* j* [i] [j] -> i j i* j*
    error = temp + 1 - 2*backend.real(_compute_overlap_1(T1, T2, W, T1_prime, T2_prime, W_prime, U))
...
def tebd_step_1(T1, T2, W, U, mode="svd", debug_logger=debug_logging.DebugLogger(), **kwargs):
    ...
    Parameters
    ----------
    T1: backend.array_type of shape (i, r1, ld, lu)
        part of the two-site wavefunction
    T2: backend.array_type of shape (j, ru, rd, l1)
        part of the two-site wavefunction
    W: backend.array_type of shape (l, u, r, d) = (r1, u, l2, d)
        part of the two-site wavefunction
    U: backend.array_type of shape (i, j, i, j)
        two-site (time evolution) operator.
    ...
    **kwargs:
    ...
    Returns
    -------
    T1_prime : backend.array_type of shape (i, r1, ld, lu)
        part of the updated two-site wavefunction
    T2_prime : backend.array_type of shape (j, ru, rd, l1)
        part of the updated two-site wavefunction
...
def tebd_step_iterate_polar_1(T1, T2, W, U, N_iters=100):
    ...
    def _update_T1(T1, T2, W, T2_prime, W_prime, U):
        ...
        T1, T2, W: backend.array_types
            two site wavefunction psi. For the shapes of the individual tensors, see tebd_step_1().
        T2_prime, W_prime: backend.array_types
            two site wavefunction psi_prime. For the shapes of the individual tensors, see tebd_step_1().
        U: backend.array_type of shape (i, j, i, j)
            time evolution operator
        ...
        Returns
        -------
        T1_prime: backend.array_type of shape (i, r, (ld, lu))
            updated T1_prime tensor.
        """
        # Flip arrow of W_prime
        l, ud, r = W_prime.shape
        W_prime = W_prime.reshape(l, ud*r) # l, (u, d), r -> l, ((u, d), r)
        W_prime, _ = backend.qr(W_prime.T, mode="reduced") # l, ((u, d), r) -> ((u, d), r), l
        W_prime = W_prime.reshape(ud, r, l) # (u, d), r, l
        # Contract everything except T1_prime
        contr = backend.tensordot(T2, backend.conj(T2_prime), ([1], [1])) # j [(ru rd)] l; j* [(ru rd)*] l* -> j l j* l*
        contr = backend.tensordot(contr, U, ([0, 2], [3, 1])) # [j] l [j*] l*: i [j] i* [j*] -> l l* i i*
        contr = backend.tensordot(contr, W, ([0], [2])) # [l] l* i i*; l (u d) [r] -> l* i i* l (u d)
        contr = backend.tensordot(backend.conj(W_prime), contr, ([0, 1], [4, 0])) # [(u d)*] [r*] l*; [l*] i i* l [(u d)] -> l* i i* l
        contr = backend.tensordot(contr, T1, ([2, 3], [0, 1])) # l* i [i*] [l]; [i] [r] (ld lu) -> l* i (ld lu) = r i (ld lu)
        # Isometrize
        r, i, ld_lu = contr.shape
        contr = contr.reshape(r, i*ld_lu) # r, i, (ld, lu) -> r, (i, (ld, lu))
        contr, _ = backend.qr(contr.T, mode="reduced") # r, (i, (ld, lu)) -> (i, (ld, lu)), r
    ...
    def _update_W(T1, T2, W, T1_prime, T2_prime, U):
        ...
        Returns
        -------
        W_prime: backend.array_type of shape (l, (u, d), r)
            updated W_prime tensor.
        """
        # Contract everything except W_prime
        contr = _compute_overlap_except_W_prime_1(T1, T2, W, T1_prime, T2_prime, U)
        # normalize
        return contr / backend.norm(contr)
    ...
    def _update_T2(T1, T2, W, T1_prime, W_prime, U):
        ...
        T1, T2, W: backend.array_types
            two site wavefunction psi. For the shapes of the individual tensors, see tebd_step_1().
        T1_prime, W_prime: backend.array_types
            two site wavefunction psi_prime. For the shapes of the individual tensors, see tebd_step_1().
        U: backend.array_type of shape (i, j, i, j)
            time evolution operator
        ...
        Returns
        -------
        T2_prime: backend.array_type of shape (j, (ru, rd), l)
            updated T2_prime tensor.
        """
        # Flip arrow of W_prime
        l, ud, r = W_prime.shape
        W_prime = W_prime.reshape(l*ud, r) # l, (u, d), r -> (l, (u, d)), r
        W_prime, _ = backend.qr(W_prime, mode="reduced")
        W_prime = W_prime.reshape(l, ud, r) # (l, (u, d)), r -> l, (u, d), r
        # Contract everything except T2_prime
        contr = backend.tensordot(T1, backend.conj(T1_prime), ([2], [2])) # i r [(ld lu)]; i* r* [(ld lu)*] -> i r i* r*
        contr = backend.tensordot(contr, U, ([0, 2], [2, 0])) # [i] r [i*] r*; [i] j [i*] j* -> r r* j j*
        contr = backend.tensordot(contr, W, ([0], [0])) # [r] r* j j*; [l] (u d) r -> r* j j* (u d) r
        contr = backend.tensordot(contr, backend.conj(W_prime), ([0, 3], [0, 1])) # [r*] j j* [(u d)] r; [l*] [(u d)*] r* -> j j* r r*
        contr = backend.tensordot(contr, T2, ([1, 2], [0, 2])) # j [j*] [r] r*; [j] (ru rd) [l] -> j r* (ru rd) = j l (ru rd)
        # Isometrize
        j, l, ru_rd = contr.shape
        contr = contr.transpose(0, 2, 1) # j, l, (ru rd) -> j, (ru, rd), l
        contr = contr.reshape(j*ru_rd, l) # j, (ru, rd), l -> (j, (ru, rd)), l
        contr, _ = backend.qr(contr, mode="reduced")
...
def _compute_overlap_except_W_prime_2(T1, T2, Wm1, W, Wp1, T1_prime, T2_prime, Wm1_prime, Wp1_prime, U):
    ...
    if Wm1 is None:
        if Wp1 is None:
            contr = backend.tensordot(T2, backend.conj(T2_prime), ([1, 3], [1, 3])) # p2 [r2] ld2 [lu2]; p2* [r2*] ld2* [lu2*] -> p2 ld2 p2* ld2*
            ...
            temp = backend.tensordot(T1, backend.conj(T1_prime), ([2, 3], [2, 3])) # p1 ru1 [rd1] [l1]; p1* ru1* [rd1*] [l1*] -> p1 ru1 p1* ru1*
            contr = backend.tensordot(temp, contr, ([0, 1, 2], [2, 3, 1])) # [p1] [ru1] [p1*] ru1*; ld2* [i] [i*] [l] u d -> ru1* ld2* u d = l r u d
            contr = backend.transpose(contr, (0, 2, 1, 3)) # l, r, u, d -> l, u, r, d
        else:
            contr = backend.tensordot(Wp1, backend.conj(Wp1_prime), ([0, 1], [0, 1])) # [lp1] [up1] rp1 dp1; [lp1*] [up1*] rp1* dp1* -> rp1 dp1 rp1* dp1*
            temp = backend.tensordot(T2, backend.conj(T2_prime), ([1], [1])) # p2 [r2] ld2 lu2; p2* [r2*] ld2* lu2* -> p2 ld2 lu2 p2* ld2* lu2*
            ...
            temp = backend.tensordot(T1, backend.conj(T1_prime), ([2, 3], [2, 3])) # p1 ru1 [rd1] [l1]; p1* ru1* [rd1*] [l1*] -> p1 ru1 p1* ru1*
            contr = backend.tensordot(temp, contr, ([0, 1, 2], [3, 4, 2])) # [p1] [ru1] [p1*] ru1*; dp1* ld2* [i] [i*] [l] d -> ru1* dp1* ld2* d = l u r d
    else:
        if Wp1 is None:
            contr = backend.tensordot(Wm1, backend.conj(Wm1_prime), ([2, 3], [2, 3])) # lm1 um1 [rm1] [dm1]; lm1* um1* [rm1*] [dm1*] -> lm1 um1 lm1* um1*
            temp = backend.tensordot(T1, backend.conj(T1_prime), ([3], [3])) # p1 ru1 rd1 [l1]; p1* ru1* rd1* [l1*] -> p1 ru1 rd1 p1* ru1* rd1* 
            ...
            temp = backend.tensordot(T2, backend.conj(T2_prime), ([1, 3], [1, 3])) # p2 [r2] ld2 [lu2]; p2* [r2*] ld2* [lu2*] -> p2 ld2 p2* ld2*
            contr = backend.tensordot(contr, temp, ([2, 3, 5], [2, 0, 1])) # um1* ru1* [j] [j*] u [r]; [p2] [ld2] [p2*] ld2* -> um1* ru1* u ld2* = d l u r
            contr = backend.transpose(contr, (1, 2, 3, 0)) # d, l, u, r -> l, u, r, d
        else:
            contr = backend.tensordot(Wp1, backend.conj(Wp1_prime), ([0, 1], [0, 1])) # [lp1] [up1] rp1 dp1; [lp1*] [up1*] rp1* dp1* -> rp1 dp1 rp1* dp1*
            temp = backend.tensordot(T2, backend.conj(T2_prime), ([1], [1])) # p2 [r2] ld2 lu2; p2* [r2*] ld2* lu2* -> p2 ld2 lu2 p2* ld2* lu2*
            temp = backend.tensordot(temp, U, ([0, 3], [3, 1])) # [p2] ld2 lu2 [p2*] ld2* lu2*; i [j] i* [j*] -> ld2 lu2 ld2* lu2* i i*
            ...
            contr = backend.tensordot(contr, W, ([0, 2], [1, 2])) # [dp1] dp1* [ld2] ld2* i i*; l [u] [r] d -> dp1* ld2* i i* l d
            temp = backend.tensordot(T1, backend.conj(T1_prime), ([3], [3])) # p1 ru1 rd1 [l1]; p1* ru1* rd1* [l1*] -> p1 ru1 rd1 p1* ru1* rd1* 
            temp2 = backend.tensordot(Wm1, backend.conj(Wm1_prime), ([2, 3], [2, 3])) # lm1 um1 [rm1] [dm1]; lm1* um1* [rm1*] [dm1*] -> lm1 um1 lm1* um1*
            temp = backend.tensordot(temp, temp2, ([2, 5], [0, 2])) # p1 ru1 [rd1] p1* ru1* [rd1*]; [lm1] um1 [lm1*] um1* -> p1 ru1 p1* ru1* um1 um1*
            contr = backend.tensordot(temp, contr, ([0, 1, 2, 4], [3, 4, 2, 5])) # [p1] [ru1] [p1*] ru1* [um1] um1*; dp1* ld2* [i] [i*] [l] [d] -> ru1* um1* dp1* ld2*
            contr = backend.transpose(contr, (0, 2, 3, 1)) # ru1*, um1*, dp1*, ld2* = l, d, u, r -> l, u, r, d
    return contr
...
def _compute_error_2(T1, T2, Wm1, W, Wp1, T1_prime, T2_prime, Wm1_prime, W_prime, Wp1_prime, U):
    ...
    """
    # temp should be 1, but compute it just to make sure.
    temp = _compute_overlap_2(T1, T2, Wm1, W, Wp1, T1_prime, T2_prime, Wm1_prime, W_prime, Wp1_prime, backend.tensordot(U, backend.conj(U), ([2, 3], [2, 3]))) # i j [i*] [j*]; i* j* [i] [j] -> i j i* j*
    error = temp + 1 - 2*backend.real(_compute_overlap_2(T1, T2, Wm1, W, Wp1, T1_prime, T2_prime, Wm1_prime, W_prime, Wp1_prime, U))
    error = backend.sqrt(error)
    return error
...
def tebd_step_iterate_polar_2(T1, T2, Wm1, W, Wp1, U, N_iters):
    ...
    def _update_T1(T1, T2, Wm1, W, Wp1, T2_prime, Wm1_prime, W_prime, Wp1_prime, U):
        ...
        Returns
        -------
        T1_prime: backend.array_type of shape (i, ru1, rd1, l1)
            updated T1_prime tensor.
        """
        ...
        if Wm1 is None:
            ...
            contr = backend.tensordot(contr, T1, ([1, 2], [0, 1])) # i [i*] [l] l*; [p1] [ru1] rd1 l1 -> i l* rd1 l1 = p1 ru1 rd1 l1
            T1_prime = backend.transpose(contr, (0, 3, 2, 1)) # p1, ru1, rd1, l1 -> p1, l1, rd1, ru1
            ...
            contr = backend.tensordot(contr, T1, ([0, 3, 4], [2, 0, 1])) # [lm1] lm1* i [i*] [l] l*; [p1] [ru1] [rd1] l1 -> lm1* i l* l1
            T1_prime = backend.transpose(contr, (1, 3, 0, 2)) # lm1*, i, l*, l1 = rd1, p1, ru1, l1 -> p1, l1, rd1, ru1
            p1, ru1, rd1, l1 = U.shape[2], W_prime.shape[0], Wm1_prime.shape[0], T1.shape[3]
        # isometrize T1
        if Wm1 is None:
            T1_prime = backend.reshape(T1_prime, (p1*l1*rd1, ru1)) # p1, l1, rd1, ru1 -> (p1, l1, rd1), ru1
            ...
            T1_prime = utility.isometrize_polar(T1_prime)
            T1_prime = backend.reshape(T1_prime, (p1, l1, rd1, ru1)) # (p1, l1), (rd1, ru1) -> p1, l1, rd1, ru1
        T1_prime = backend.transpose(T1_prime, (0, 3, 2, 1)) # p1, l1, rd1, ru1 -> p1, ru1, rd1, l1
        return T1_prime
    ...
    def _update_T2(T1, T2, Wm1, W, Wp1, T1_prime, Wm1_prime, W_prime, Wp1_prime, U):
        ...
        Returns
        -------
        T2_prime: backend.array_type of shape (j, r2, ld2, lu2)
            updated T2_prime tensor.
        """
        # Contract everything except T2
        if Wm1 is None:
            contr = backend.tensordot(T1, backend.conj(T1_prime), ([2, 3], [2, 3])) # p1 ru1 [l1] [lu1]; p1* ru1* [rd1*] [l1*] -> p1 ru1 p1* ru1*
            ...
            if Wp1 is None:
                ...
                contr = backend.tensordot(contr, T2, ([1, 2], [0, 2])) # j [j*] [r] r*; [p2] r2 [ld2] lu2 -> j r* r2 lu2 = p2 ld2 r2 lu2
                T2_prime = backend.transpose(contr, (0, 2, 1, 3)) # p2, ld2, r2, lu2 -> p2, r2, ld2, lu2
                ...
                contr = backend.tensordot(contr, T2, ([1, 2, 4], [0, 2, 3])) # j [j*] [r] r* [rp1] rp1*; [p2] r2 [ld2] [lu2] -> j r* rp1* r2
                T2_prime = backend.transpose(contr, (0, 3, 1, 2)) # j, r*, rp1*, r2 = p2, ld2, lu2, r2 -> p2, r2, ld2, lu2
        ...
        # Isometrize T2
        if Wp1 is None:
            T2_prime = backend.transpose(T2_prime, (0, 1, 3, 2)) # p2, r2, ld2, lu2 -> p2, r2, lu2, ld2
            T2_prime = backend.reshape(T2_prime, (p2*r2*lu2, ld2)) # p2, r2, lu2, ld2 -> (p2, r2, lu2), ld2
            T2_prime = utility.isometrize_polar(T2_prime)
            T2_prime = backend.reshape(T2_prime, (p2, r2, lu2, ld2)) # (p2, r2, lu2), ld2 -> p2, r2, lu2, ld2
            T2_prime = backend.transpose(T2_prime, (0, 1, 3, 2)) # p2, r2, lu2, ld2 -> p2, r2, ld2, lu2
        else:
            T2_prime = backend.reshape(T2_prime, (p2*r2, ld2*lu2)) # p2, r2, ld2, lu2 -> (p2, r2), (ld2, lu2)
            T2_prime = utility.isometrize_polar(T2_prime)
            T2_prime = backend.reshape(T2_prime, (p2, r2, ld2, lu2)) # (p2, r2), (ld2, lu2) -> p2, r2, ld2, lu2
        return T2_prime
    ...
    def _update_Wp1(T1, T2, Wm1, W, Wp1, T1_prime, T2_prime, Wm1_prime, W_prime, U):
        ...
        Returns
        -------
        Wp1_prime: backend.array_type of shape (lp1, up1, rp1, dp1)
        ...
        if Wm1 is None:
            contr = backend.tensordot(T1, backend.conj(T1_prime), ([2, 3], [2, 3])) # p1 ru1 [rd1] [l1]; p1* ru1* [rd1*] [l1*] -> p1 ru1 p1* ru1*
            contr = backend.tensordot(contr, U, ([0, 2], [2, 0])) # [p1] ru1 [p1*] ru1*; [i] j [i*] j* -> ru1 ru1* j j*
            contr = backend.tensordot(contr, W, ([0], [0])) # [ru1] ru1* j j*; [l] u r d -> ru1* j j* u r d
            contr = backend.tensordot(contr, backend.conj(W_prime), ([0, 5], [0, 3])) # [ru1*] j j* u r [d]; [l*] u* r* [d*] -> j j* u r u* r*
        else:
...
def tebd_step_2(T1, T2, Wm1, W, Wp1, U, chi_max, mode="svd", debug_logger=debug_logging.DebugLogger(), **kwargs):
    ...
    Parameters
    ----------
    T1 : backend.array_type of shape (i, ru1, rd1, l1)
        part of the two-site wavefunction
    T2 : backend.array_type of shape (j, r2, ld2, lu2)
        part of the two-site wavefunction
